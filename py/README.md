...
The Python SDK for the MunicipalFinance API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.AgedCreditor(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.MunicipalFinance(None).load(
    {"id": "test01"}, None
)
