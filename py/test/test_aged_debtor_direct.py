...
class TestAgedDebtorDirect:

    def test_should_direct_list_aged_debtor(self):
        setup = _aged_debtor_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-aged_debtor", "live" if setup["live"] else "unit")
        if _skip:
...
def _aged_debtor_direct_setup(mockres):
