# MunicipalFinance SDK test runner
...
class MunicipalFinanceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MunicipalFinanceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MunicipalFinanceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MunicipalFinanceTestRunner.getenv("MUNICIPAL_FINANCE_TEST_LIVE")
        override = MunicipalFinanceTestRunner.getenv("MUNICIPAL_FINANCE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MunicipalFinanceTestRunner.getenv(key)
        ...
        explain = MunicipalFinanceTestRunner.getenv("MUNICIPAL_FINANCE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MUNICIPAL_FINANCE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MunicipalFinanceTestRunner._test_control is not None:
            return MunicipalFinanceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MunicipalFinanceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MunicipalFinanceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MunicipalFinanceTestRunner.load_test_control()
...
def load_env_local():
    MunicipalFinanceTestRunner.load_env_local()


def env_override(m):
    return MunicipalFinanceTestRunner.env_override(m)


def entity_data(v):
    return MunicipalFinanceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MunicipalFinanceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MunicipalFinanceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MunicipalFinanceTestRunner.load_test_control()


def live_client_options():
    return MunicipalFinanceTestRunner.live_client_options()


def live_delay_ms():
    return MunicipalFinanceTestRunner.live_delay_ms()
