...
// MunicipalFinance SDK feature test
//
...
class FtCtrl extends MunicipalFinanceControl
{
...
class FtClient
{
    ...
    private MunicipalFinanceUtility $utility;

    public function __construct(MunicipalFinanceUtility $utility, array $options)
    {
    ...
    public function get_utility(): MunicipalFinanceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MunicipalFinanceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MunicipalFinanceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MunicipalFinanceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MunicipalFinanceConfig::make_feature($name);
    ...
    private static function buildUrl(MunicipalFinanceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MunicipalFinanceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MunicipalFinanceResponse($fetched);
        }
    ...
    private function fail(MunicipalFinanceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MunicipalFinanceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
