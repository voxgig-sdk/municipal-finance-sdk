...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MunicipalFinanceOmni.make_runner(TEST_JSON_FILE, MunicipalFinanceSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    MunicipalFinanceError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MunicipalFinanceOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = MunicipalFinanceOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
