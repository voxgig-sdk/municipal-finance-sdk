...
// MunicipalFinance SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MunicipalFinanceOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MunicipalFinanceOmni::structfix($now, Struct::undef()))
                        !== json_encode(MunicipalFinanceOmni::structfix($val, Struct::undef()))) {
