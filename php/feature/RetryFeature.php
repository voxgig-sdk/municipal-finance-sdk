...
// MunicipalFinance SDK retry feature
...
class MunicipalFinanceRetryFeature extends MunicipalFinanceBaseFeature
{
    ...
    public function init(MunicipalFinanceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MunicipalFinanceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MunicipalFinanceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MunicipalFinanceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
