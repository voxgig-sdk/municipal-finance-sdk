...
func TestAgedDebtorEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.AgedDebtor(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.AgedDebtor(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MUNICIPAL_FINANCE_TEST_AGED_DEBTOR_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		agedDebtorRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.aged_debtor")))
...
func aged_debtorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUNICIPAL_FINANCE_TEST_AGED_DEBTOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUNICIPAL_FINANCE_TEST_AGED_DEBTOR_ENTID"])
