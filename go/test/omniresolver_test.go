...
import (
	...
	omni "github.com/voxgig-sdk/municipal-finance-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MunicipalFinanceError); is {
				out := map[string]any{
					"name":    "MunicipalFinanceError",
...
func omniCtx(arg any, client *sdk.MunicipalFinanceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MunicipalFinanceSDK); is {
			client = live
