...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MUNICIPAL_FINANCE_TEST_LIVE") == "TRUE" ||
		os.Getenv("MUNICIPAL_FINANCE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MUNICIPAL_FINANCE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MunicipalFinanceSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MunicipalFinanceError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MunicipalFinanceSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MunicipalFinanceError{Msg: msg, Code: code}
}
