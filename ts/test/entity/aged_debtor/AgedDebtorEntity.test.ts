...
describe('AgedDebtorEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'aged_debtor.' + op, live)) return
    }
    ...
    const aged_debtor_ref01_list = await aged_debtor_ref01_ent.list(aged_debtor_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MUNICIPAL_FINANCE_TEST_AGED_DEBTOR_ENTID']
