...
describe('AgedCreditorDirect', async () => {
  ...
  test('direct-list-aged_creditor', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-aged_creditor', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MUNICIPAL_FINANCE_TEST_AGED_CREDITOR_ENTID']
