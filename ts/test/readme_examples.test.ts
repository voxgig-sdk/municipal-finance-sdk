...
//     (\`new MunicipalFinanceSDK(...)\` / \`MunicipalFinanceSDK.test(...)\` become
//     \`MunicipalFinanceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MunicipalFinanceSDK'
...
const TEST_SEED = {"entity":{"aged_creditor":{"test01":{"id":"test01"}},"aged_debtor":{"test01":{"id":"test01"}},"fact":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MunicipalFinanceSDK(...)\` and any
// \`MunicipalFinanceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MunicipalFinanceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MunicipalFinanceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
