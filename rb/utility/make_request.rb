...
module MunicipalFinanceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MunicipalFinanceResponse.new({})
    ...
    if fetch_err
      ...
      response = MunicipalFinanceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MunicipalFinanceResponse.new(fetched)
    else
