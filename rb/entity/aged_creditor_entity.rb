...
class AgedCreditorEntity
  ...
  def make
    opts = @_entopts.dup
    AgedCreditorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MunicipalFinanceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AgedCreditor, Hash] the current AgedCreditor data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MunicipalFinanceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AgedCreditor fields)
  def match_get
  ...
  #
  # @param reqmatch [AgedCreditorListMatch, Hash, nil] match filter (any subset of
  #   AgedCreditor fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<AgedCreditor>, Array] the matching AgedCreditor items; raises MunicipalFinanceError on failure
