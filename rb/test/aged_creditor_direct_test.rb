...
class AgedCreditorDirectTest < Minitest::Test
  def test_direct_list_aged_creditor
    setup = aged_creditor_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-aged_creditor", setup[:live] ? "live" : "unit")
    if _should_skip
...
def aged_creditor_direct_setup(mockres)
