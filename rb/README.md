# MunicipalFinance Ruby SDK



The Ruby SDK for the MunicipalFinance API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
result, err = client.AgedCreditor().list
raise err if err
...
result, err = client.MunicipalFinance().load({ "id" => "test01" })
...
```
rb/
├── MunicipalFinance_sdk.rb       -- Main SDK module
