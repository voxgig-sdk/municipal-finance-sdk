// Command MunicipalFinance-cli is a CLI + REPL for the MunicipalFinance SDK,
...
const prompt = "municipal-finance"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "aged_creditor aged_debtor fact"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MUNICIPAL_FINANCE_APIKEY (api key), MUNICIPAL_FINANCE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MUNICIPAL_FINANCE_APIKEY carries the API key and
	// MUNICIPAL_FINANCE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MUNICIPAL_FINANCE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MUNICIPAL_FINANCE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMunicipalFinanceSDK(opts)
