# municipal-finance-cli

boru-driven command-line client **and** interactive REPL for the MunicipalFinance
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/municipal-finance-cli)
make build

# 2. See usage (words, entities, env vars)
./municipal-finance-cli --help

# 3. Provide credentials once, via the environment
export MUNICIPAL_FINANCE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./municipal-finance-cli list aged_creditor
./municipal-finance-cli list aged_debtor

# 5. Override the API base URL for a single call
MUNICIPAL_FINANCE_BASE=https://api.example.com ./municipal-finance-cli list aged_creditor

# 6. No arguments -> interactive REPL
./municipal-finance-cli
municipal-finance> list aged_creditor
municipal-finance> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/municipal-finance-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/municipal-finance-cli list aged_creditor
   ```
...
Run with no arguments to open a REPL (prompt `municipal-finance>`). Each line is
evaluated as its own boru expression:

```text
$ ./municipal-finance-cli
municipal-finance> list aged_creditor
municipal-finance> /help
municipal-finance> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `aged_creditor`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/municipal-finance-cli`. |
...
(`./municipal-finance-cli <expr>`) and interactively (the REPL), and expressions compose the
