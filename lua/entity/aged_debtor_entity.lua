...
local AgedDebtorEntity = {}
AgedDebtorEntity.__index = AgedDebtorEntity


function AgedDebtorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgedDebtorEntity)
  self._name = "aged_debtor"
  self._client = client
...
function AgedDebtorEntity:get_name()
  return self._name
end


function AgedDebtorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgedDebtorEntity.new(self._client, opts)
end
...
function AgedDebtorEntity:mark_deleted()
  self._deleted = true
end


function AgedDebtorEntity:deleted()
  return true == self._deleted
end


function AgedDebtorEntity:data_set(args)
  if args ~= nil then
...
function AgedDebtorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgedDebtorEntity:match_set(args)
  if args ~= nil then
...
function AgedDebtorEntity:match_get()
...
function AgedDebtorEntity:stream(action, args, callopts)
...
function AgedDebtorEntity:_run_op(ctx, post_done)
  local utility = self._utility
