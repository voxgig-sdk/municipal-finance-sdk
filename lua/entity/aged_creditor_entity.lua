...
local AgedCreditorEntity = {}
AgedCreditorEntity.__index = AgedCreditorEntity


function AgedCreditorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgedCreditorEntity)
  self._name = "aged_creditor"
  self._client = client
...
function AgedCreditorEntity:get_name()
  return self._name
end


function AgedCreditorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgedCreditorEntity.new(self._client, opts)
end
...
function AgedCreditorEntity:mark_deleted()
  self._deleted = true
end


function AgedCreditorEntity:deleted()
  return true == self._deleted
end


function AgedCreditorEntity:data_set(args)
  if args ~= nil then
...
function AgedCreditorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgedCreditorEntity:match_set(args)
  if args ~= nil then
...
function AgedCreditorEntity:match_get()
...
function AgedCreditorEntity:stream(action, args, callopts)
...
---@param reqmatch AgedCreditorListMatch
---@param ctrl? table
---@return AgedCreditor[]
---@return string? err
function AgedCreditorEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AgedCreditorEntity:_run_op(ctx, post_done)
  local utility = self._utility
