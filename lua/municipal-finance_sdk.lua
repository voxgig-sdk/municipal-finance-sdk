...
require("municipal-finance_types")
...
local MunicipalFinanceSDK = {}
MunicipalFinanceSDK.__index = MunicipalFinanceSDK
...
MunicipalFinanceSDK._make_feature = _make_feature


function MunicipalFinanceSDK.new(options)
  local self = setmetatable({}, MunicipalFinanceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MunicipalFinanceSDK:options_map()
...
function MunicipalFinanceSDK:get_utility()
  return Utility.copy(self._utility)
end


function MunicipalFinanceSDK:get_root_ctx()
  return self._rootctx
end


function MunicipalFinanceSDK:prepare(fetchargs)
...
function MunicipalFinanceSDK:direct(fetchargs)
...
function MunicipalFinanceSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MunicipalFinanceSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MunicipalFinanceSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MunicipalFinanceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MunicipalFinanceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MunicipalFinanceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MunicipalFinanceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MunicipalFinanceSDK.new(sdkopts)
  sdk.mode = "test"
