...
The Lua SDK for the MunicipalFinance API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AgedCreditor(nil):list(nil, nil)
...
local result, err = client:MunicipalFinance(nil):load(
  { id = "test01" }, nil
)
