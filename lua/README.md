# MunicipalFinance Lua SDK



The Lua SDK for the MunicipalFinance API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AgedCreditor()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
for _, item in ipairs(agedcreditors) do
  print(item["amount_type_code"])
end
```
...
Create an instance: `local aged_creditor = client:AgedCreditor(nil)`
...
```lua
local aged_creditors, err = client:AgedCreditor():list()
```


### AgedDebtor

Create an instance: `local aged_debtor = client:AgedDebtor(nil)`
...
```lua
local aged_debtors, err = client:AgedDebtor():list()
```
...
```
lua/
├── municipal-finance_sdk.lua    -- Main SDK module
...
The main module (`municipal-finance_sdk`) exports the SDK constructor
