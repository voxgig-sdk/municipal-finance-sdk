...
MCP server exposing the MunicipalFinance SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `municipal-finance_list` | `entity`, optional `query` map | First page of records as JSON |
| `municipal-finance_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o municipal-finance-mcp ./...
```
...
After install, restart Claude Code; the `municipal-finance_list` and
`municipal-finance_load` tools become available in new sessions.
