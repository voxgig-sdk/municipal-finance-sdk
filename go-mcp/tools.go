...
type Args struct {
	Entity string         `json:"entity" jsonschema:"aged_creditor | aged_debtor | fact"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MunicipalFinanceSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "municipal-finance_list",
		Description: "List records from MunicipalFinance. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "municipal-finance_load",
		Description: "Load a single record from MunicipalFinance. " +
...
func runOp(client *sdk.MunicipalFinanceSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MunicipalFinanceSDK, name string) (sdk.MunicipalFinanceEntity, error) {
	switch strings.ToLower(name) {
	case "aged_creditor":
		return client.AgedCreditor(nil), nil
	case "aged_debtor":
		return client.AgedDebtor(nil), nil
	case "fact":
